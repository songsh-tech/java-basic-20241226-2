...
class Korean extends Human {
    ...
    // Java에서는 단일 상속만 지원함
    // 다중상속에 발생할 수 있는 다이아몬드 문제가 발생하지 않도록 하기 위함
    // 다이아몬드 문제 : 다중 상속으로 인해 동일한 이름의 메서드를 상속받은 상태에서 어떤 메서드를 선택해야할지 모호해지는 문제
    // 인터페이스를 사용하여 다중상속 효과를 볼 수 있음, 중첩 상속을 사용하여 다중 상속 효과를 볼 수 있음
...
// Java의 모든 클래스는 Object 클래스를 최상위 클래스로 상속받아 사용
class SalesMan extends Human {
	...
	// 2. 상위 클래스에서 선언된 메서드가 final로 선언되면 재정의가 불가능함
	// @Override: 컴파일러에게 현재 작업이 오버라이딩 작업임을 알려줌
	@Override
	void sleep() {
		// 오버라딩을 통해서 코드의 유연성을 증가 시킬수 있음
